...
function fromURL(url, options) {
  return Promise.resolve().then(() => {
    ...
    return request(url, requestOptions).then(res => {
      ...
      return jsdom(res.body, Object.assign(inferredOptions, options));
    });
  });
}
...
function normalizeFromURLOptions(options = {}) {
  ...
  // Normalization of options which must be done before the rest of the fromURL code can use them
  ...
  // fromURL calls jsdom(html, options).
}
